...
const handTypeSolver = (hand: string): HandScore => {
  ...
  // five of a kind
  if (foundCardFaceCount.length === 1 || jokerCount === 5) {
    return {
    ...
    if (foundCardFaceCount.some((v) => v === (4 - jokerCount))) {
      return {
        handType: handType.fourOfAKind,
        faces: foundCardFaceValues,
      };
    }
    if (foundCardFaceCount.some((v) => v === (3 - jokerCount))) {
      return {
    ...
    if (
      ...
      foundCardFaceCount.some((v) => v === 2 - jokerCount)
    ) {
      return {
